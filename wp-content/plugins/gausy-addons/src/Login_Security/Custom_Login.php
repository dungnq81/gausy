...
final class Custom_Login {
	...
	public function restrict_login_to_ips() {
		$_login_security          = gausy_theme_setting_filters( 'login_security', false );
		...
		if ( ! empty( $allowed_ips ) ) {
			foreach ( $allowed_ips as $allowed_ip ) {
				if ( $this->_ipInRange( gausy_ip_address(), $allowed_ip ) ) {
					return true;
				}
			}
			...
			error_log( 'Restricted login page: access currently not permitted - ' . gausy_ip_address() );
			wp_die(
		...
		if ( ! empty( $blocked_ips ) ) {
			foreach ( $blocked_ips as $blocked_ip ) {
				if ( $this->_ipInRange( gausy_ip_address(), $blocked_ip ) ) {
